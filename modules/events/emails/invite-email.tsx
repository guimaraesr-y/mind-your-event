...
export const EventInviteEmailTemplate: React.FC<EventInviteEmailTemplateProps> = async ({
  eventTitle,
  eventDescription,
  inviteLink,
  authorName,
        ...
        <div style={containerStyle}>
          ...
          <div style={contentStyle}>
            ...
            <div style={{
              ...
              <h3 style={{ marginTop: 0, marginBottom: '10px', fontSize: '18px' }}>{t('eventDetails')}</h3>
              <p style={{ margin: 0 }}>{eventDescription}</p>
            </div>
            <p style={{ margin: '0 0 15px' }}>{t('confirmPresence')}</p>
            <a href={inviteLink} style={buttonStyle}>
              {t('rsvpButton')}
            </a>
            <p style={{ fontSize: '12px', color: 'var(--muted-foreground, #6b7280)', marginTop: '20px' }}>
              {t('linkNotWorking')} <a href={inviteLink} style={{ color: 'var(--primary, #3b82f6)' }}>{t('clickHere')}</a>
            </p>
          </div>
